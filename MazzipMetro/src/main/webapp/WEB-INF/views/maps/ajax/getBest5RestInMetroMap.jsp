...
<script type="text/javascript"
	src="<%=request.getContextPath()%>/resources/tooltipster/dist/js/tooltipster.bundle.min.js"></script>
<link rel="stylesheet" type="text/css"
	href="<%=request.getContextPath()%>/resources/tooltipster/dist/css/tooltipster.bundle.min.css" />
<link rel="stylesheet" type="text/css"
	href="<%=request.getContextPath()%>/resources/tooltipster/dist/css/plugins/tooltipster/sideTip/themes/tooltipster-sideTip-noir.min.css" />
...
table.tbl_best5, table.tbl_best5 th, table.tbl_best5 td {
	border: solid 1px navy;
	border-collapse: collapse;
}

table.tbl_best5 th, table.tbl_best5 td {
	padding: 5px;
}
</style>
<script>
								...
								.on(
										...
										function(event) {
											...
											if ($(
													event.instance
															.elementOrigin())
													.hasClass('tooltip_group')) {

												var instances = $.tooltipster
														.instances('.tooltip_group'), open = false, duration;
...
<body>
	...
	<c:if
		test="${not empty places || not empty tags || not empty masterUser }">
		<!-- 베스트 음식점 정보 -->
		<div style="position: relative; float: left; margin: 10px;"
			align="center">
			<h3 style="padding-bottom: 5px;">
				<span style="font-weight: bold;">${metroName}</span> 맛집 베스트5
			</h3>
			<table class="tbl_best5">
				<c:forEach var="vo" items="${places}" varStatus="status" begin="0"
					end="4">
					<tr>
						...
						<a
							href="<%=request.getContextPath()%>/restaurantDetail.eat?restSeq=${vo.restSeq}"
							data-tooltip-content="#tooltip_content_rest${status.index}"
							class='tooltipster tooltip_group'
							style="color: black; text-decoration: none;"> <span
							style='font-weight: bold; font-size: 18px;'>${vo.restName}</span>
		...
		<c:if test="${reviews != null && not empty reviews}">
			<div
				style="position: relative; float: left; margin-left: 20px; margin: 10px;"
				align="center">
				<h3 style="padding-bottom: 5px;">
					<span style="font-weight: bold;">${metroName}</span> 마스터즈 5
				</h3>
				<table class="tbl_best5">
					<c:forEach var="vo" items="${reviews}" varStatus="status" begin="0"
						end="4">
						<tr>
							<th style="color: lime;">${status.count}</th>
							<c:if test="${status.count%2 == 0 }">
							...
							<a href="#"
								data-tooltip-content="#tooltip_content_review${status.index}"
								class='tooltipster tooltip_group'
								style="color: black; text-decoration: none;"> <span
								style='font-weight: bold; font-size: 18px;'>${vo.userName}</span>
								/ <span style="color: #ff1a75; font-size: 14px;">${vo.gradeName}</span>
							</a>
							</td>
						</tr>
					</c:forEach>
		...
		<div class="tooltip_templates">
			<c:forEach var="vo" items="${places}" varStatus="status" begin="0"
				...
				<div id="tooltip_content_rest${status.index}">
					<div id="div_tooltipImg">
						<img src="<%=request.getContextPath()%>/files/rest/${vo.restImg}"
							width="500px;" />
					</div>
			...
			<c:forEach var="vo" items="${bestReview}" varStatus="status"
				begin="0" end="4">
						...
						<table>
							<tr>
								<td rowspan="2"><img
									...
									width="50px;" /></td>
								<td style="padding-left: 15px; vertical-align: middle;"><b>${vo.userName}</b></td>
