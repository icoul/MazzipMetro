...
<style>
input.btn_like {
	width:200px; height:100px;  background: url('http://localhost:9090/mazzipmetro/resources/images/like.png') no-repeat;
}
</style>
				...
				<td>
					...
					<c:forEach var="likers" items="${likers}">
						<c:if test="${likers == review.reviewSeq }">
							<p align="right">
								<input type="button" class="btn_like" id="like${status.index}" name="like" value="${review.reviewHit}Hit!" onClick="DownHit('${review.reviewSeq}','like${status.index}');" /><!-- style="background-color: white; color: black" --> 
							</p>
						...
						<c:if test="${flag eq 0}">
							<p align="right">
								<input type="button" class="btn_like" id="like${status.index}" name="like" value="${review.reviewHit}Hit!" onClick="insertAndUpHit('${review.reviewSeq}','like${status.index}');" />
							</p>
						</c:if>
